...
JS;
    ...
    protected function getFormFrontValue()
    {
        switch (get_class($this)) {
            ...
            case Radio::class:
                return <<<JS
var checked = \$this.closest('.fields-group').find('{$this->getElementClassSelector()}:checked').val();
JS;
            case Checkbox::class:
                return <<<JS
var checked = \$this.closest('.fields-group').find('{$this->getElementClassSelector()}:checked').map(function(){
  return $(this).val();
}).get();
JS;
            default:
